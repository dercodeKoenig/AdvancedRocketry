...
public class TestProbeCommand extends CommandBase {
        ...
        try {
            switch (args[0].toLowerCase()) {
                ...
                case "event":
                    ...
                    break;
                case "fakeplayer":
                    handleFakePlayer(server, sender, tail(args));
                    break;
                default:
    ...
    // TASK-10 — FakePlayer probes. Headless dedicated-server tests have
    // no real connected player; many AR production paths take an
    // EntityPlayer argument and behave differently per-player. The
    // FakePlayerFactory (Forge built-in) returns a singleton fake
    // player per (world, GameProfile) pair — we cache them by name in
    // a static map so subsequent probe calls operate on the same entity.
    private static final java.util.Map<String, net.minecraftforge.common.util.FakePlayer> FAKE_PLAYERS =
            new java.util.concurrent.ConcurrentHashMap<>();

    private void handleFakePlayer(net.minecraft.server.MinecraftServer server,
                                  ICommandSender sender, String[] args) {
        if (args.length == 0) {
            send(sender, "{\"error\":\"usage: /artest fakeplayer create|teleport|tick|destroy|info|list <args>\"}");
            return;
        }
        ...
        if ("create".equals(sub) && args.length >= 2) {
            ...
            String name = args[1];
            int dim = args.length >= 3 ? parseIntOr(args[2], 0) : 0;
            net.minecraft.world.WorldServer world = server.getWorld(dim);
            if (world == null) {
                send(sender, "{\"error\":\"world not loaded\",\"dim\":" + dim + "}");
                return;
            }
            java.util.UUID uuid = java.util.UUID.nameUUIDFromBytes(("ARFakePlayer:" + name).getBytes());
            com.mojang.authlib.GameProfile profile = new com.mojang.authlib.GameProfile(uuid, name);
            net.minecraftforge.common.util.FakePlayer fp =
                    net.minecraftforge.common.util.FakePlayerFactory.get(world, profile);
            FAKE_PLAYERS.put(name, fp);
            send(sender, "{\"ok\":true,\"name\":\"" + escapeJson(name)
                    + "\",\"uuid\":\"" + uuid + "\",\"dim\":" + dim + "}");
            return;
        }
        if ("teleport".equals(sub) && args.length >= 6) {
            ...
            net.minecraftforge.common.util.FakePlayer fp = FAKE_PLAYERS.get(name);
            if (fp == null) {
                send(sender, "{\"error\":\"fake player not found — create it first\",\"name\":\""
                        + escapeJson(name) + "\"}");
                return;
            }
            net.minecraftforge.common.DimensionManager.keepDimensionLoaded(dim, true);
            ...
            int fromDim = fp.world.provider.getDimension();
            if (fromDim != dim) {
                // FakePlayer doesn't go through PlayerList.transferPlayerToDimension
                // cleanly (no client packets). Move it manually: remove
                // from current world's entity list, attach to new world,
                // refresh GameProfile binding.
                fp.world.removeEntity(fp);
                fp.dimension = dim;
                fp.setWorld(destWorld);
                destWorld.spawnEntity(fp);
                // Re-fetch the per-world singleton so subsequent .get(world)
                // returns this same instance.
                FAKE_PLAYERS.put(name,
                        net.minecraftforge.common.util.FakePlayerFactory.get(destWorld,
                                fp.getGameProfile()));
                fp = FAKE_PLAYERS.get(name);
            }
            // setPositionAndUpdate calls into the network handler which
            // for a FakePlayer is null (no client connection) and NPEs.
            // setLocationAndAngles is the server-only position setter.
            fp.setLocationAndAngles(x, y, z, fp.rotationYaw, fp.rotationPitch);
            send(sender, "{\"ok\":true,\"name\":\"" + escapeJson(name)
                    + "\",\"fromDim\":" + fromDim + ",\"toDim\":" + dim
                    + ",\"x\":" + x + ",\"y\":" + y + ",\"z\":" + z + "}");
            return;
        }
        if ("tick".equals(sub) && args.length >= 2) {
            // /artest fakeplayer tick <name> [count] — drives
            // EntityPlayer.onUpdate via direct invocation.
            String name = args[1];
            int count = args.length >= 3 ? parseIntOr(args[2], 1) : 1;
            net.minecraftforge.common.util.FakePlayer fp = FAKE_PLAYERS.get(name);
            if (fp == null) {
                send(sender, "{\"error\":\"fake player not found\",\"name\":\""
                        + escapeJson(name) + "\"}");
                return;
            }
            int ticked = 0;
            try {
                ...
                send(sender, "{\"error\":\"fakeplayer.onUpdate threw after " + ticked
                        + " ticks: " + escapeJson(e.getClass().getSimpleName() + ": "
                        + e.getMessage()) + "\"}");
                return;
            }
            send(sender, "{\"ok\":true,\"ticked\":" + ticked + "}");
        ...
        if ("destroy".equals(sub) && args.length >= 2) {
            String name = args[1];
            net.minecraftforge.common.util.FakePlayer fp = FAKE_PLAYERS.remove(name);
            if (fp != null && fp.world != null) {
                fp.world.removeEntity(fp);
            }
            send(sender, "{\"ok\":true,\"removed\":" + (fp != null) + "}");
            return;
        }
        if ("info".equals(sub) && args.length >= 2) {
            String name = args[1];
            net.minecraftforge.common.util.FakePlayer fp = FAKE_PLAYERS.get(name);
            if (fp == null) {
                send(sender, "{\"error\":\"fake player not found\",\"name\":\""
            ...
            Map<String, Object> info = new LinkedHashMap<>();
            info.put("name", name);
            info.put("uuid", fp.getUniqueID().toString());
            info.put("dim", fp.world.provider.getDimension());
            info.put("posX", fp.posX);
            info.put("posY", fp.posY);
            info.put("posZ", fp.posZ);
            info.put("air", fp.getAir());
            info.put("health", fp.getHealth());
            info.put("isAlive", fp.isEntityAlive());
            info.put("ticksExisted", fp.ticksExisted);
            send(sender, jsonMap(info));
            return;
        }
        if ("fire-living-update".equals(sub) && args.length >= 2) {
            // /artest fakeplayer fire-living-update <name> — posts
            // LivingUpdateEvent on the EVENT_BUS for this fake player.
            // PlanetEventHandler.playerTick is subscribed to this event;
            // firing it lets tests drive the per-tick player logic
            // (force-teleport-out-of-spaceDim, advancement triggers,
            // gravity handling) without needing the World.updateEntities
            // chunk-tick path.
            String name = args[1];
            net.minecraftforge.common.util.FakePlayer fp = FAKE_PLAYERS.get(name);
            if (fp == null) {
            ...
            int dimBefore = fp.world.provider.getDimension();
            double posXBefore = fp.posX;
            double posZBefore = fp.posZ;
            // AR's player-tick subscribers (PlanetEventHandler.playerTick,
            // AtmosphereHandler.onTick) call into PlayerList.transferPlayer-
            // ToDimension and EntityPlayerMP.onNewPotionEffect — both NPE
            // on a FakePlayer (no client connection). Production assumes
            // real players. Swallow any subscriber exception here; the
            // observable side-effects (entity.dimension, position) are
            // still set BEFORE the NPE in most listeners, so we can read
            // them back. The `subscriberThrew` field surfaces whether the
            // event-post completed cleanly or not.
            String subscriberError = "";
            try {
                net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(
                        new net.minecraftforge.event.entity.living.LivingEvent.LivingUpdateEvent(fp));
            } catch (Throwable t) {
                subscriberError = t.getClass().getSimpleName()
                        + (t.getMessage() != null ? ": " + t.getMessage() : "");
            }
            // Re-fetch the fake player after the event — production
            // PlanetEventHandler may have called transferPlayerToDimension
            // which moves the entity to a different world; the cached
            // FAKE_PLAYERS map ref must be refreshed for subsequent
            // probe calls to find it.
            int dimAfter = fp.world.provider.getDimension();
            if (dimAfter != dimBefore) {
                net.minecraft.world.WorldServer newWorld =
                        server.getWorld(dimAfter);
                if (newWorld != null) {
                    FAKE_PLAYERS.put(name,
                            net.minecraftforge.common.util.FakePlayerFactory.get(newWorld,
                                    fp.getGameProfile()));
                }
            }
            Map<String, Object> out = new LinkedHashMap<>();
            out.put("ok", true);
            out.put("dimBefore", dimBefore);
            out.put("dimAfter", dimAfter);
            out.put("posXBefore", posXBefore);
            out.put("posXAfter", fp.posX);
            out.put("posZBefore", posZBefore);
            out.put("posZAfter", fp.posZ);
            out.put("subscriberError", subscriberError);
            send(sender, jsonMap(out));
            return;
        }
        if ("list".equals(sub)) {
            ...
            boolean first = true;
            for (String n : FAKE_PLAYERS.keySet()) {
                if (!first) b.append(',');
                first = false;
                b.append('"').append(escapeJson(n)).append('"');
            }
            b.append("]}");
            send(sender, b.toString());
            return;
        }
        send(sender, "{\"error\":\"unknown fakeplayer subcommand — try create|teleport|tick|destroy|info|list\"}");
    }
